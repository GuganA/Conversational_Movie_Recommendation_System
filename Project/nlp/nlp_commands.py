...
import random
import json
from config import service, location
...
def handle_command(message, channel, message_user,context):
    ...
    response = service.message(workspace_id='02f445f1-6022-4938-816d-63bbc0b76553', input = {'text':message}, context = context).get_result()
    ...
    try:
        ...
        #search_key = str(response['entities'][0]['value'])
    except:
        search_key = ''
    
    try:
        search_term = str(response['context']['movie_name'])
        #search_term = response['context']['movie_name']
    except:
        search_term = ''
