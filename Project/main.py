...
if __name__ == "__main__":
    if slack_client.rtm_connect(with_team_state=False):
        print("Sam connected and running!")
        ...
        while True:
            ...
            if message: # If a User has typed something in Slack
                ...
                string_to_run = string.Template("""python -W ignore "${log_commands_path}" "${user_id}" "${message_user}" "${conversation_id}" "${message}" "${slack_output}" "${team}" "${channel}" "${start_timestamp}" "${end_timestamp}" "${processing_time}" "${follow_ind}" &""").substitute(locals()) #logs processing 
